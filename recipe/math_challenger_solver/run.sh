...
IH_PARQUET="${GSM8K_INITIAL_HISTORY:-/root/autodl-tmp/verl/data/gsm8k/train.parquet}"
...
python3 -m recipe.math_challenger_solver.main_ppo \
  ...
  data.train_files="${DATA_TRAIN:-/root/autodl-tmp/verl/data/gsm8k/test.parquet}" \
  data.val_files="${DATA_VAL:-/root/autodl-tmp/verl/data/gsm8k/test.parquet}" \
  data.train_batch_size=1024 \
  data.max_prompt_length=1024 \
  data.max_response_length=1024 \
  data.filter_overlong_prompts=True \
  data.truncation='error' \
  actor_rollout_ref.model.path="${MODEL_PATH:-/root/autodl-tmp/models/Qwen2.5-1.5B-Instruct}" \
