...
用法：
    conda run -n verl python recipe/RLSD/diagnostic/classify_problems.py \
        --pass_at_k /data3/yyy/verl/data/mrsd/pass_at_k_results.jsonl \
        --context_ab /data3/yyy/verl/data/mrsd/diagnostic/context_ab_results.jsonl \
...
def parse_args():
    ...
    p.add_argument(
        "--context_ab",
        default="/data3/yyy/verl/data/mrsd/diagnostic/context_ab_results.jsonl",
    )
    p.add_argument(
        "--output_dir",
...
def main():
    ...
    from recipe.RLSD.mrsd.prompt import build_student_messages, build_teacher_context_b
